...
router.post('/findGoods',(req,res) => {
    ...
    models.Goods.find({"phone":params.phone}).then((result)=>{
        res.send(result)
    })}else{models.Goods.find().then((result)=>{
        res.send(result)
    })}
...
router.post('/cancelGoods',(req,res) => {
    let params = req.body
    models.Goods.updateOne({"randomNum":params.goods},{"condition":"已取消"}).then((result)=>{res.send(true)})
...
router.post('/updateGoods',(req,res) => {
    ...
    if(params.condition==="待派单"){
        ...
        models.Goods.updateOne({"randomNum":params.goods},{"condition":"已验收"}).then((result)=>{res.send(true)})
    }
